...
if (theme_get_setting('goosebumps_rebuild_registry') && !defined('MAINTENANCE_MODE')) {
  // Rebuild .info data.
 ...
 */
function goosebumps_theme(&$existing, $type, $theme, $path) {
  include_once './' . drupal_get_path('theme', 'goosebumps') . '/template.theme-registry.inc';
  return _goosebumps_theme($existing, $type, $theme, $path);
}
 ...
 */
function goosebumps_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  $output = '';

  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('goosebumps_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('goosebumps_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = filter_xss_admin(theme_get_setting('goosebumps_breadcrumb_separator'));
      $trailing_separator = $title = '';
      if (theme_get_setting('goosebumps_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('goosebumps_breadcrumb_trailing')) {
 ...
 *   also be "maintenance_page" since goosebumps_preprocess_maintenance_page() calls
 *   this function to have consistent variables.
 */
function goosebumps_preprocess_html(&$variables, $hook) {
  // Add variables and paths needed for HTML5 and responsive support.
  $variables['base_path'] = base_path();
  $variables['path_to_goosebumps'] = drupal_get_path('theme', 'goosebumps');
 ...
 */
function goosebumps_process_html(&$variables, $hook) {
  // Flatten out html_attributes.
  $variables['html_attributes'] = drupal_attributes($variables['html_attributes_array']);
}

function goosebumps_get_node_count($content_type) {
 ...
 */
function goosebumps_process_html_tag(&$variables) {
 ...
 */
function goosebumps_html_head_alter(&$head) {
 ...
 */
function goosebumps_preprocess_maintenance_page(&$variables, $hook) {
  goosebumps_preprocess_html($variables, $hook);
  // There's nothing maintenance-related in goosebumps_preprocess_page(). Yet.
  //goosebumps_preprocess_page($variables, $hook);
}
 ...
 *   The name of the template being rendered ("maintenance_page" in this case.)
 */
function goosebumps_process_maintenance_page(&$variables, $hook) {
  goosebumps_process_html($variables, $hook);
 ...
 */
function goosebumps_preprocess_node(&$variables, $hook) {
 ...
 */
function goosebumps_preprocess_comment(&$variables, $hook) {
 ...
 */
function goosebumps_preprocess_region(&$variables, $hook) {
  // Sidebar regions get some extra classes and a common template suggestion.
  if (strpos($variables['region'], 'sidebar_') === 0) {
    $variables['classes_array'][] = 'column';
    $variables['classes_array'][] = 'sidebar';
    // Allow a region-specific template to override Goosebumps' region--sidebar.
  ...
  elseif ($variables['region'] == 'content') {
    // Allow a region-specific template to override Goosebumps' region--no-wrapper.
 ...
 */
function goosebumps_preprocess_block(&$variables, $hook) {
  ...
  // The last_in_region property is set in goosebumps_page_alter().
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function goosebumps_process_block(&$variables, $hook) {
 ...
 */
function goosebumps_page_alter(&$page) {
 ...
 */
function goosebumps_form_node_form_alter(&$form, &$form_state, $form_id) {
 ...
 * @ingroup themeable
 */
function goosebumps_menu_local_tasks(&$variables) {
  $output = '';
 ...
 * @ingroup themeable
 */
function goosebumps_menu_local_task($variables) {
  $type = $class = FALSE;

  $link = $variables['element']['#link'];
  $link_text = $link['title'];

  // Check for tab type set in goosebumps_menu_local_tasks().
 ...
 */
function goosebumps_preprocess_menu_link(&$variables, $hook) {
 ...
 */
function goosebumps_status_messages($variables) {
 ...
 */
function goosebumps_mark($variables) {
  $type = $variables['type'];
...
function goosebumps_wysiwyg_editor_settings_alter(&$settings, $context) {
